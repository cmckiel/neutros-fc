...
bool mpu6050_set_fs_sel_gyro(mpu6050_fs_sel_gyro_t fs_select)
{
	...
	txn.i2c_op = HAL_I2C_OP_WRITE;
	txn.expected_bytes_to_tx = 2;
	txn.tx_data[0] = MPU_GYRO_CONFIG_REG;
	txn.tx_data[1] |= (fs_select << MPU_GYRO_CONFIG_FS_SEL_Shift);
...
bool mpu6050_calc_gyro_offsets(int16_t *xg_offset, int16_t *yg_offset, int16_t *zg_offset)
{
	...
	for (int i = 0; i < MPU_OFFSET_AVERAGE_SAMPLE_SIZE; ++i)
	{
		...
		// message to burst read gryo
		txn.i2c_op = HAL_I2C_OP_WRITE_READ;
		txn.expected_bytes_to_tx = 1;
		txn.expected_bytes_to_rx = 6;
		txn.tx_data[0] = MPU_GYRO_XOUT_H_REG;
		...
		xg_data[i] = (txn.rx_data[0] << 8) | txn.rx_data[1];
		yg_data[i] = (txn.rx_data[2] << 8) | txn.rx_data[3];
		zg_data[i] = (txn.rx_data[4] << 8) | txn.rx_data[5];
	}
	...
	for (int i = 0; i < MPU_OFFSET_AVERAGE_SAMPLE_SIZE; ++i)
	{
		xg_sum += (int32_t)xg_data[i];
		yg_sum += (int32_t)yg_data[i];
		zg_sum += (int32_t)zg_data[i];
	}

	int32_t xg_avg = xg_sum / MPU_OFFSET_AVERAGE_SAMPLE_SIZE;
	int32_t yg_avg = yg_sum / MPU_OFFSET_AVERAGE_SAMPLE_SIZE;
	int32_t zg_avg = zg_sum / MPU_OFFSET_AVERAGE_SAMPLE_SIZE;

	// Check for null
	if (!xg_offset || !yg_offset || !zg_offset)
	{
		return false;
	}

	// return the average
	*xg_offset = (int16_t)xg_avg;
	*yg_offset = (int16_t)yg_avg;
	*zg_offset = (int16_t)zg_avg;
